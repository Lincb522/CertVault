      ...
      <el-col :xs="24" :sm="14">
        <div class="content-card">
          ...
          <div style="margin-bottom: 16px">
            <el-select v-model="selectedAccountId" placeholder="选择绑定的开发者账号" style="width: 100%" :loading="loadingAccounts" filterable>
              <el-option v-for="acc in accounts" :key="acc.id" :label="acc.name" :value="acc.id" />
            </el-select>
            <p style="color: var(--nask-text-secondary); font-size: 12px; margin-top: 4px">获取 UDID 后将自动绑定到所选账号，无需设备端登录</p>
          </div>

          <!-- 未生成 -->
          <div v-if="!requestId" class="empty-state">
            <HIcon name="display-1" :size="48" />
            <p style="margin-top: 12px">选择账号后点击「生成新链接」</p>
            <p style="color: var(--nask-text-secondary); font-size: 13px">设备端获取 UDID 后将自动签名绑定，无需再次登录</p>
          </div>
...
import { udidApi, accountApi } from '../api'
...
async function createRequest() {
  ...
  try {
    const res = await udidApi.createRequest(selectedAccountId.value)
