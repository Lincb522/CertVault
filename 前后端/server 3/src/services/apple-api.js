...
class AppleApiService {
  ...
  async request(method, endpoint, data = null, params = null) {
    ...
    if (params && typeof params === 'object') {
      ...
      fullUrl += sep + new URLSearchParams(params).toString();
    }
    const config = {
      method,
      url: fullUrl,
  ...
  async updateBetaGroup(groupId, attributes = {}) {
  ...
  // ---- Beta Recruitment Criteria (device/OS conditions for public links) ----
  async getBetaRecruitmentCriteria(groupId) {
    try {
      return await this.request('GET', `/betaGroups/${groupId}/betaRecruitmentCriteria`);
    } catch (e) {
      return { data: null };
    }
  }

  async createBetaRecruitmentCriteria(groupId, attributes = {}) {
    return this.request('POST', '/betaRecruitmentCriteria', {
      data: {
  ...
  async updateBetaRecruitmentCriteria(criteriaId, attributes = {}) {
    return this.request('PATCH', `/betaRecruitmentCriteria/${criteriaId}`, {
      data: { type: 'betaRecruitmentCriteria', id: criteriaId, attributes }
    });
  }

  async deleteBetaRecruitmentCriteria(criteriaId) {
    return this.request('DELETE', `/betaRecruitmentCriteria/${criteriaId}`);
  }
  ...
  async listGroupTesters(groupId, params = {}) {
    const defaults = {
    ...
    return this.request('GET', `/betaGroups/${groupId}/betaTesters${query ? '?' + query : ''}`);
  }
  ...
  async getBetaAppReviewSubmission(buildId) {
    return this.request('GET', `/builds/${buildId}/betaAppReviewSubmission`);
  }
  ...
  async updateBuildBetaDetail(betaDetailId, attributes = {}) {
    return this.request('PATCH', `/buildBetaDetails/${betaDetailId}`, {
      data: { type: 'buildBetaDetails', id: betaDetailId, attributes }
    });
  }

  // ---- Beta App Review Detail (app-level test info) ----
  async getBetaAppReviewDetail(appId) {
    const result = await this.request('GET', `/apps/${appId}/betaAppReviewDetail`);
    return result;
  }

  async updateBetaAppReviewDetail(detailId, attributes = {}) {
    return this.request('PATCH', `/betaAppReviewDetails/${detailId}`, {
      data: { type: 'betaAppReviewDetails', id: detailId, attributes }
    });
  }

  // ---- Beta License Agreement ----
  async getBetaLicenseAgreement(appId) {
    return this.request('GET', `/apps/${appId}/betaLicenseAgreement`);
  }

  async updateBetaLicenseAgreement(agreementId, agreementText) {
    return this.request('PATCH', `/betaLicenseAgreements/${agreementId}`, {
      data: { type: 'betaLicenseAgreements', id: agreementId, attributes: { agreementText } }
    });
  }
  ...
  async setBuildExportCompliance(buildId, usesNonExemptEncryption = false) {
    ...
    if (!betaDetailId) throw new Error('无法获取构建 Beta 详情');
    return this.request('PATCH', `/buildBetaDetails/${betaDetailId}`, {
      data: {
