...
// push-keys routes are now integrated into push.js under /push/keys/*
...
// push-keys now served under /api/push/keys/* (integrated into pushRoutes)
...
// ---- Custom HTML page for Public TF Share ----
app.get('/admin/tf/s/:slug', (req, res, next) => {
  const tfSharePath = path.join(publicDir, 'tf-share.html');
  if (fs.existsSync(tfSharePath)) {
    return res.sendFile(tfSharePath);
  }
  next();
});
// ---------------------------------------------
...
initDatabase().then(async () => {
  ...
  const cleanupExpiredTrials = async () => {
    ...
    const db = getDb();
    try {
      const expiredUsers = await db.prepare('SELECT * FROM tf_trial_users WHERE expires_at <= NOW()').all();
        ...
        for (const row of expiredUsers) {
          try {
            ...
            if (account) {
              ...
              await api.removeTesterFromGroup(row.group_id, [row.tester_id]).catch(e => {
                if (e.status !== 404 && e.status !== 409) throw e;
              });
              await api.deleteBetaTester(row.tester_id).catch(e => {
                if (e.status !== 404 && e.status !== 409) throw e;
              });
            }
            await db.prepare('DELETE FROM tf_trial_users WHERE id = ?').run(row.id);
            console.log(`[TF Trial] 已自动清理过期体验测试人员: ${row.email}`);
          } catch (err) {
            console.warn(`清理过期体验测试人员 ${row.email} 失败:`, err.message);
            if (err.status === 404) {
              await db.prepare('DELETE FROM tf_trial_users WHERE id = ?').run(row.id);
              console.log(`[TF Trial] 账号不存在或已被删除，清理异常记录: ${row.email}`);
            }
          }
        }
      }
  ...
  setInterval(cleanupExpiredTrials, 60_000);
