  ...
  try {
    ...
    try {
      ...
      if (tokenAdminPassword) {
        const testerName = [first_name, last_name].filter(Boolean).join(' ').trim() || email.split('@')[0];
        const http = require('http');
        const postData = JSON.stringify({ email, name: testerName, source: 'tf-invite' });
        ...
        console.log('[TF→Token] 自动注册:', email, tokenResult.exists ? '(已存在)' : '(新建)', 'key:', tokenResult.key);
      }
    } catch (tokenErr) {
      console.warn('[TF→Token] 自动注册失败:', tokenErr.message);
    }
