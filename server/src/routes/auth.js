...
async function ensureAdminExists() {
  ...
  const requireAdminConfig = () => {
    if (!adminUsername || !adminPassword || adminPassword.includes('CHANGE_ME')) {
      throw new Error('ADMIN_USERNAME and ADMIN_PASSWORD must be configured before the first start.');
    }
  };
  ...
  if (count.c === 0) {
    ...
    await db.prepare('INSERT INTO users (id, username, password, salt, role, email_verified) VALUES (?, ?, ?, ?, ?, 1)')
      .run(crypto.randomUUID(), adminUsername, hash, salt, 'superadmin');
    ...
    if (!superadmin) {
      requireAdminConfig();
      const existing = await db.prepare('SELECT * FROM users WHERE username = ?').get(adminUsername);
      if (existing) {
