...
if (!pgPassword || pgPassword.includes('CHANGE_ME')) {
  throw new Error('PG_PASSWORD is required. Copy .env.example to .env and set a secure database password.');
}

const pool = new Pool({
  host: process.env.PG_HOST || '127.0.0.1',
  port: parseInt(process.env.PG_PORT || '5432'),
  database: process.env.PG_DATABASE || 'certvault',
  user: process.env.PG_USER || 'certvault',
  password: pgPassword,
  max: 10,
});
...
async function initDatabase() {
  ...
  await pool.query(`
    ...
    CREATE TABLE IF NOT EXISTS tf_trial_users (
      id SERIAL PRIMARY KEY,
      group_id TEXT NOT NULL,
    ...
    CREATE INDEX IF NOT EXISTS idx_tf_trial_expires ON tf_trial_users(expires_at);

    CREATE TABLE IF NOT EXISTS tf_trial_email_history (
      id SERIAL PRIMARY KEY,
      email TEXT NOT NULL,
    ...
    CREATE UNIQUE INDEX IF NOT EXISTS idx_tf_trial_email_history_normalized ON tf_trial_email_history(normalized_email);
  `);
