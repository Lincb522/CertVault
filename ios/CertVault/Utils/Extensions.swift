...
// MARK: - View Modifiers (legacy, will be replaced per-view)
...
extension View {
    ...
    func cardStyle() -> some View {
        if #available(iOS 26, *) {
            self
                ...
                .glassEffect(.regular.tint(.dsSurface.opacity(0.5)), in: .rect(cornerRadius: DS.radiusLG))
        } else {
            self
                .padding(DS.spacingLG)
    ...
    @ViewBuilder
    func glassCard(cornerRadius: CGFloat = DS.radiusLG) -> some View {
        if #available(iOS 26, *) {
    ...
    @ViewBuilder
    func glassInteractive(tint: Color? = nil, cornerRadius: CGFloat = DS.radiusMD) -> some View {
        if #available(iOS 26, *) {
