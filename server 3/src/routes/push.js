...
router.post('/history/:id/resend', pushEnabledMiddleware, async (req, res, next) => {
  try {
    const db = getDb();
    const item = await db.prepare('SELECT * FROM push_history WHERE id = ?').get(req.params.id);
    if (!item) return res.status(404).json({ success: false, message: '历史记录不存在' });

    const settings = req.pushSettings || await getSettings();
    const title = item.title;
    const messageBody = item.body || '';
    const bundle_id = item.bundle_id || settings.default_bundle_id;
    const sandbox = item.sandbox;

    if (!title || !bundle_id) {
      return res.status(400).json({ success: false, message: '原始记录缺少标题或 Bundle ID，无法重发' });
    }

    const pushKeyId = settings.default_push_key_id;
    if (!pushKeyId) {
      return res.status(400).json({ success: false, message: '未配置默认推送密钥' });
    }

    let creds;
    try { creds = await resolvePushCredentials({ push_key_id: pushKeyId }, req.user); } catch (e) {
      return res.status(e.status || 400).json({ success: false, message: e.message });
    }

    const priority = parseInt(settings.apns_priority || '10', 10);
    const expiration = settings.apns_expiration || '0';
    const payload = APNsService.buildPayload({ title, body: messageBody, sound: 'default' });
    ...
    if (item.type === 'broadcast') {
      ...
      const msg = `重发广播完成：${results.success} 成功，${results.failed} 失败` +
        (results.unregistered > 0 ? `，${results.unregistered} 已注销` : '');
      res.json({ success: true, message: msg, data: { total: devices.length, ...results } });
    } else {
      const device_token = item.device_token;
      if (!device_token) {
        return res.status(400).json({ success: false, message: '原始记录缺少 device_token，无法重发单推' });
      }

      const result = await apnsService.send(device_token, payload, {
        ...creds, bundleId: bundle_id, sandbox: sandbox ?? false, priority, expiration,
      });
      ...
      const reason = result.body?.reason;
      const reasonCN = ok ? '重发推送成功' : getReasonCN(reason);
      if (ok) {
        res.json({ success: true, message: '重发推送成功', data: { apns_id: result.apnsId, status: result.status, reason_cn: reasonCN } });
      } else {
        res.json({ success: false, message: `重发失败: ${reasonCN}`,
          data: { status: result.status, reason, reason_cn: reasonCN, apns_id: result.apnsId } });
      }
    }
  } catch (err) { next(err); }
