...
function renderOneResource(r){
  var label=r.bundle_identifier||r.profile_name||'签名包';
  var h='<div style="font-size:13px;font-weight:600;margin-bottom:4px">'+label+'</div>';
  ...
  h+='</div><div class="esign-result"></div>';
  return h;
}

function renderExistResources(res){
  var pw=res[0]?res[0].cert_password||'123456':'123456';
  var h='<div class="bind-panel"><div class="card"><div class="bind-title">'+ic('check',16)+' <span style="color:var(--green)">此设备已绑定签名</span></div>';
  h+=renderOneResource(res[0]);
  if(res.length>1){
    h+='<div style="margin-top:14px;border-top:1px solid var(--border);padding-top:12px">';
    h+='<div class="tut-hd" onclick="toggleTut(this)" style="padding:8px 0;margin:0">';
    h+='<span style="font-size:13px;color:var(--text-muted);font-weight:600">历史签名包 ('+(res.length-1)+')</span>';
    h+='<span class="tut-arrow">▼</span></div>';
    h+='<div class="tut-body"><div style="padding-top:8px">';
    for(var i=1;i<res.length;i++){
      h+='<div style="padding:10px 0;border-top:1px solid var(--border)">'+renderOneResource(res[i])+'</div>';
    }
    h+='</div></div></div>';
  }
  h+='</div></div>';
